...
write_text.c         ...*本文を画面に出力するための関数(枠内を超えた場合は別関数)
...
・ファイル整頓
・以前の解決項目の削除
・枠内を超える文字サイズの場合に使用されるwrite_text_new_lineを追加。
・テキストの全角と半角を区別可能にした
・file_openを様々な場所で使用可能にした
・テキスト側からAAを読み込み表示させるようにした(カレントフォルダからの位置なので、適宜修正)
　(char *ではコアダンプするが事前にサイズを与えるとコアダンプしない)

・メニュー画面を指定のものにする
・コマンドライン引数からテキストを選ぶようにする
・write_textの関数の類似部分を関数化する
・文字プロット中にgetchの入力を受け付けないようにする
・画面上にミニメッセージを描画できるようにする
・geditのKscripter用のプラグインの作成
・メニュー画面をカーソルで位置を把握できるようにする
・バックログとセーブとポーズで再描画しない
